...
While it's possible to use the methods below to set a configuration option directly, their main intended purpose is to be used as internal plumbing and to provide an escape hatch in case there's no better alternatives.

For better alternatives, please take a look at L<Rex::Commands> first, which provides convenience wrappers for many of these configuration settings.
